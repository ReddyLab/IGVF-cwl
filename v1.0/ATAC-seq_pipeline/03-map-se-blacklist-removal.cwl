...
cwlVersion: v1.2
class: Workflow
doc: 'ATAC-seq 03 mapping - reads: SE - blacklist removal'
requirements:
...
inputs:
  ...
  genome_ref_first_index_file:
    doc: Bowtie first index files for reference genome (e.g. *1.ebwt). The rest of
  ...
  percent_mitochondrial_reads:
    ...
    in:
      ...
      chrom:
        valueFrom: chrM
      output_filename:
        valueFrom: ${return inputs.idxstats.basename.replace(/^.*[\\\/]/, '').replace(/\.[^/.]+$/,
          '').replace(/\.[^/.]+$/, '').replace(/\.[^/.]+$/, '.mitochondrial_percentage.txt')}
    out:
    - percent_map
outputs:
  output_pbc_files:
