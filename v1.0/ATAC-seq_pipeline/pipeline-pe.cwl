...
cwlVersion: v1.2
class: Workflow
doc: 'ATAC-seq pipeline - reads: PE'
requirements:
...
outputs:
  ...
  map_mark_duplicates_files:
    doc: Summary of duplicates removed with Picard tool MarkDuplicates (for multiple
      reads aligned to the same positions
    type: File[]
  ...
  peakcall_output_unpaired_peak_xls_file:
    doc: Peak calling report file (*_peaks.xls file produced by MACS2) using each
      paired mate independently
    type: File[]
    outputSource: peak_call/output_unpaired_peak_xls_file
  peakcall_output_unpaired_extended_peak_file:
    doc: peakshift/phantomPeak extended fragment results file using each paired mate
      independently
    type: File[]
