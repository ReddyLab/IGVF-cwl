...
inputs:
  ...
  ENCODE_blacklist_bedfile:
    ...
    type: File
  regions_bed_file:
    doc: Regions bed file used to filter-in reads (used in samtools, for example chromosomes
      of interest)
    type: File
  genome_ref_first_index_file:
    doc: '"First index file of Bowtie2 reference genome with extension 1.bt2. \ (Note:
