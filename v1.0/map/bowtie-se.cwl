...
inputs:
  ...
  genome_ref_first_index_file:
    ...
    doc: First file (extension .1.ebwt) of the Bowtie index files generated for the
      reference genome (see http://bowtie-bio.sourceforge.net/tutorial.shtml#newi)
  trim5:
    type: int?
    inputBinding:
      position: 1
  ...
  chunkmbs:
    ...
    inputBinding:
      position: 5
      prefix: --chunkmbs
    doc: 'The number of megabytes of memory a given thread is given to store path
      descriptors in --best mode. (Default: 256)'
  best:
    type: boolean
    default: true
    inputBinding:
