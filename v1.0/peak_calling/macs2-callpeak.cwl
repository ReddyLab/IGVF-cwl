...
inputs:
  ...
  extsize:
    ...
    doc: "The arbitrary extension size in bp. When nomodel is  true, MACS will use
      this value as fragment size to  extend each read towards 3' end, then pile them
      up.  It's exactly twice the number of obsolete SHIFTSIZE.  In previous language,
      each read is moved 5'->3'  direction to middle of fragment by 1/2 d, then  extended
      to both direction with 1/2 d. This is  equivalent to say each read is extended
      towards 5'->3'  into a d size fragment. DEFAULT: 200. EXTSIZE and  SHIFT can
      be combined when necessary. Check SHIFT  option."
  verbose:
    type: int?
    inputBinding:
      position: 1
...
# ------------------------------------- #
  ...
  format:
    ...
    doc: "-f {AUTO,BAM,SAM,BED,ELAND,ELANDMULTI,ELANDEXPORT,BOWTIE,BAMPE}, --format
      ...
      \"AUTO\", \"BED\" or \"ELAND\" or \"ELANDMULTI\" or \"ELANDEXPORT\" or \"SAM\"\
      \ or \"BAM\" or \"BOWTIE\" or \"BAMPE\". The default AUTO option will let MACS
      decide which format the file is. Note that MACS can't detect \"BAMPE\" or \"\
      BEDPE\" format with \"AUTO\", and you have to implicitly specify the format
  ...
  shift:
    ...
    doc: "(NOT the legacy --shiftsize option!) The arbitrary shift in bp. Use discretion
      ...
      other than 0 if format is BAMPE for paired-end data. DEFAULT: 0."
  m:
    type: string?
    inputBinding:
      position: 1
